...
**Status — Phases 1–3 complete, plus v1.1 through v1.39 (last updated 2026-09-08):**
...
| v1.25 · Offer & dispute race guards | A systematic sweep for the same check-then-insert shape found two more instances: `post_offer` could leave two pending offers on one match if both parties posted at the same instant, and `raise_dispute` could open two disputes on one deal under a double-submit — both closed with partial unique indexes | ✅ |
...
| v1.39 · Production-scale load test | Closed the other disclosed gap: not k6/Locust (a new dependency unjustified at this scale) but a real concurrency ramp (`scripts/load_test.py`, httpx + `ThreadPoolExecutor` like the existing `perf_bench.py`, ramping 5→120 concurrent requests instead of one fixed load point) against a live single-worker instance on real seeded Postgres data. Found the deployment's actual saturation point: throughput plateaus around 85–150 req/s (varies with machine load) while p99 latency climbs sharply past concurrency≈20-40 — the classic queueing signature of a server at its processing ceiling. Two concrete hypotheses were tested rather than assumed — raising Starlette's default 40-thread sync-endpoint limit, and widening the DB connection pool from 15 to 40 connections — and both were **ruled out**: re-running the identical ramp afterward showed no measurable change, so both speculative changes were reverted instead of kept as an unverified fix. Confirms the ceiling is per-request compute cost on one worker, not a tunable thread/connection limit — real evidence for the documented single-VM/single-worker tradeoff, not just an assertion. Also added a small, deliberately-bounded write-path check (`PATCH /api/lots/{id}`, round-robined across the 4 demo farmer accounts to stay under their own rate limit rather than polluting the shared demo dataset with throwaway accounts) | ✅ |
