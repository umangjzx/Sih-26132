      ...
      <section className="rounded-2xl border border-[var(--line)] bg-white p-6 shadow-sm">
        ...
        >
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label htmlFor="profile-name" className="flex flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]">
              {t("name")}
              <input id="profile-name" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} />
            </label>
            <label htmlFor="profile-phone" className="flex flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]">
              {t("phone")}
              <input id="profile-phone" value={user.phone} disabled className={`${inputCls} opacity-60`} />
            </label>
            <label htmlFor="profile-district" className="flex flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]">
              {t("district")}
              <input id="profile-district" value={district} onChange={(e) => setDistrict(e.target.value)} placeholder={t("districtPh")} className={inputCls} />
            </label>
            <label htmlFor="profile-state" className="flex flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]">
              {t("state")}
              <input id="profile-state" value={state} onChange={(e) => setState(e.target.value)} className={inputCls} />
            </label>
          </div>
      ...
      {user.role !== "admin" && (
        <section className="rounded-2xl border border-[var(--line)] bg-white p-6 shadow-sm">
          ...
          {v === "verified" ? (
            ...
            >
              <label htmlFor="profile-v-ref" className="flex flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]">
                {t("vRefLabel")}
                <input id="profile-v-ref" value={vRef} onChange={(e) => setVRef(e.target.value)} placeholder={t("vRefPh")} className={inputCls} />
              </label>
              <label htmlFor="profile-v-note" className="flex flex-col gap-1.5 text-sm font-semibold text-[var(--ink)]">
                {t("vNoteLabel")}
                <textarea id="profile-v-note" value={vNote} onChange={(e) => setVNote(e.target.value)} rows={2} className={inputCls} />
              </label>
              <button
                type="submit"
                disabled={saving}
