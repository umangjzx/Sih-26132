...
const CHECKLIST: { item: string; done: boolean; note: string }[] = [
  ...
  { item: "Security response headers (CSP/HSTS/X-Frame-Options)", done: true, note: "v1.38 — a global middleware sets CSP (`default-src 'none'`, exempted only on /docs so Swagger's CDN assets still load), HSTS, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, and Permissions-Policy on every response" },
