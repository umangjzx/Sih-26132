...
const SCORES = [
  {
    ...
    pct: 95,
    evidence: "Calculated: production build compiles clean across 31 routes, 554/554 automated tests pass, zero raw-SQL/injection risk found in a full-codebase grep. Docked 5% for the doc staleness this audit found and corrected.",
  },
  {
  ...
  },
  {
    ...
    pct: 87,
    evidence: "Manually assessed from: PBKDF2/JWT/RBAC/rate-limiting/input-validation all verified in code, plus a bandit static-analysis scan (7 findings, all fixed) and two rounds of pip-audit dependency scanning run 2026-09-04 — the second round surfaced after replacing python-jose+ecdsa with PyJWT, whose initial version itself had known CVEs, resolved by upgrading to PyJWT 2.13.0. Both scanners re-run 2026-09-07 against the newer feature work and still report clean. Baseline security-response headers (CSP/HSTS/X-Frame-Options/etc.) shipped 2026-09-08 — docked only for no dynamic scanner (OWASP ZAP) against a live deployment.",
  },
  {
...
const WHY = [
  "554 of 554 automated tests pass — verified by running both suites live, not quoted from a stale document",
