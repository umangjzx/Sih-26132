...
const FAQS = [
  ...
  { q: "Is this fully implemented?", a: "19 of 20 planned modules are implemented and tested (see Core Modules) — including a v1.7 dispute-resolution feature that was found undocumented in our own README during this audit and is now corrected here. The Cordova Android wrap is the one remaining gap, explicitly marked Planned, not hidden." },
  ...
  { q: "How secure is the system?", a: "PBKDF2 (600k iterations) + JWT access/refresh separation + role- and record-level authorization + a 27-site rate limiter + zero raw SQL found in the codebase. Static analysis (bandit) and dependency-CVE scanning (pip-audit) have both been run and are clean — see Security for what's still not covered (a dynamic scanner against a live deployment)." },
  ...
  { q: "What is your future roadmap?", a: "Cordova Android wrap (the frontend is already built for it — nearly every route is a pure client component); satellite crop-health via Google Earth Engine (credentials are scaffolded but unused); a k6/Locust run against an actual deployed instance, now that a local concurrency-ramp load test has already found and characterized this single-worker deployment's real saturation point (see Performance)." },
